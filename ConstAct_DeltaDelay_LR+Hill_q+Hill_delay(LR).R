##Code for 
## q=f^n/(k^n+f^n), n=1, 2, 3
## Test function for the delay depending on both f and LR size: delay=delay1+delay2*f/LR
...
Data1 <- read.csv("LastUndetFirstDet_C5.csv", header=TRUE)
...
nq=1
nd=1
AvgPnorbd_CD4_f <- function(x,IPDA,kq,delay1,delay2,kd,a,t){# we have IPDA data but no CD4
  f=x[1]
  lCD4= x[2]
  ...
  q=f^nq/(kq^nq+f^nq)
  delay=delay1+delay2*(1-LR^nd/(kd^nd+LR^nd)) 
  return(exp(-a*(1-q)*LR*(t-delay))*pdf_CD4*(1/f_det))
}

AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,kq,delay1,delay2,kd,a,t){ 
  f=x[1]
  IPDA=x[2]
  lCD4=x[3]
...
nloglik <- function(params,FirstDet,LastUndet, IPDA_Data, CD4_Data, 
  ...
  if (is.na(t1)) {
    ...
    if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #no CD4 Data, yes detectable IPDA, no neut Abs-- double integral
      pvr1 <-  1- adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(0,15),
                                 ...
                                 kq=params[1],delay1=params[2],delay2=params[3],
                                 kd=params[4],a=params[5],t=t1)$integral
  ...
  if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #IPDA, no CD4 or f-- double integral 
    pvr2 <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(0,15),
                             upperLimit=c(f_det,25),IPDA=IPDA,
                             kq=params[1],delay1=params[2],delay2=params[3],
                             kd=params[4],a=params[5],t=t2)$integral
...
### optimization time
x0de <- list(label=c("kq","delay1","delay2","kd","a"),est=c(0.01,5,32,0.1,0.01),
             low=c(1e-14,1e-14,1e-14,1e-14,1e-14),upp=c(100,300,300,100,20))
  ...
  for(i in 1:animals){
    if(is.na(CD4[i]) & !is.na(IPDA[i]) & is.na(f[i])){ #IPDA, no CD4 or f-- double integral 
      pvr[jj, i] <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(0,15),
                                     ...
                                     kq=kq,delay1=delay1,delay2=delay2,kd=kd,a=a,t=ts[jj])$integral
