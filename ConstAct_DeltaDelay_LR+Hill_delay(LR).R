...
n=2
AvgPnorbd_CD4_f <- function(x,IPDA,a,delay1,delay2,k,t){# we have IPDA data but no CD4
  f=x[1]
  lCD4= x[2]
  ...
  delay=delay1+delay2*(1-LR^n/(k^n+LR^n)) 
  return(exp(-a*LR*(t-delay))*pdf_CD4*(1/f_det))
}

AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,delay1,delay2,k,t){
  f=x[1]
  IPDA=x[2]
  lCD4=x[3]
...
### optimization time
x0de <- list(label=c("a","delay1","delay2","k"),est=c(0.01,5,5,0.01),
             low=c(1e-14,1e-14,1e-14,1e-14),upp=c(20,300,300,100))
...
for (jj in c(1:length(ts))) {
  ...
  for(i in 1:animals){
    if(is.na(CD4[i]) & !is.na(IPDA[i]) & is.na(f[i])){ #IPDA, no CD4 or f-- double integral 
      ...
      delay=delay1+delay2*(1-LR[i]^n/(k^n+LR[i]^n)) 
