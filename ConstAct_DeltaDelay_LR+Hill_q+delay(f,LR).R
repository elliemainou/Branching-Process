...
n=3
AvgPnorbd_CD4_f <- function(x,IPDA,a,k,delay1,delay2,t){# we have IPDA data but no CD4
  f=x[1]
  lCD4= x[2]
...
AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,k,delay1,delay2,t){
  f=x[1]
  IPDA=x[2]
  lCD4=x[3]
...
# AvgPnorbd_CD4_f <- function(x,IPDA,a,k,delay1,delay2,delay3,t){# we have IPDA data but no CD4
#   f=x[1]
#   lCD4= x[2]
#   LR=log10(exp(lCD4)*IPDA/1e6)  #LR size over the entire monkey, take the log10
#   pdf_CD4=(1/(sigma*sqrt(2*pi)))*exp(-(((lCD4)-mu)^2)/(2*sigma^2))
...
# }
# 
# AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,k,delay1,delay2,delay3,t){
#   f=x[1]
#   IPDA=x[2]
#   lCD4=x[3]
#   LR=log10(exp(lCD4)*IPDA/1e6)
...
### optimization time
x0de <- list(label=c("a","k","delay1","delay2"),est=c(0.01,0.01,5,5),
             low=c(1e-14,1e-14,1e-14,1e-14),upp=c(10,2,300,300))
# x0de <- list(label=c("a","k","delay1","delay2","delay3"),est=c(0.01,0.1,4,2,1),
