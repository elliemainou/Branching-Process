...
### optimization time
x0de <- list(label=c("a","q0","delay1","delay2"),est=c(0.01,0.9,1,3),
             low=c(1e-14,1e-14,1e-14,1e-14),upp=c(20,2,100,100))
  ...
  for(i in 1:animals){
    if(is.na(CD4[i]) & !is.na(IPDA[i]) & is.na(f[i])){ #IPDA, no CD4 or f-- double integral 
      ...
      pvr[jj, i] <- 1- adaptIntegrate(AvgPnorbd_CD4_IPDA_f, lowerLimit=c(1e-4,1e-2,15),
                                      ...
                                      a=a,q0=q0,delay1=delay1,delay2=delay2,t=ts[jj])$integral #Det_IPDA_Data
      if (pvr[jj,i]<0) {pvr[jj,i]=0}
