...
AvgPnorbd <- function(f,a,q0,delay1,delay2,k,t){#no rebound function
  q=min(q0*f,1)
  delay=delay1+ delay2*f^n/(k^n+f^n)
  return(exp(-a*(1-q)*(t-delay))*1/f_det)
}
...
nloglik <- function(params,FirstDet,LastUndet, Neut_Percent) {
  ...
  if (is.na(t1)) {
    ...
    if(is.na(f)){
      pvr1 <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=params[1],
                            q0=params[2],delay1=params[3],
                            delay2=params[4],k=params[5],t=t1)$value
    } else {
      q=min(q0*f,1)
      delay=delay1+ delay2*f^n/(k^n+f^n)
      pvr1 <- 1 - exp(-a*(1-q)*(t1-delay)) 
    }
  }
  if (pvr1<0) {pvr1=0}
  ...
  if(is.na(f)){
    pvr2 <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=params[1],
                          q0=params[2],delay1=params[3],
                          delay2=params[4],k=params[5],t=t2)$value
  } else {
    q=min(q0*f,1)
    delay=delay1+ delay2*f^n/(k^n+f^n)
    pvr2 <- 1 - exp(-a*(1-q)*(t2-delay))  
  }
  if (pvr2<0) {pvr2=0}
...
### optimization time
x0de <- list(label=c("a","q0","delay1","delay2","k"),est=c(0.1,0.5,2,2,0.1),
             low=c(1e-14,1e-14,1e-14,1e-14,1e-14),upp=c(5,2,200,200,100))
...
for (jj in c(1:length(ts))) {
  ...
  for(i in 1:animals){
    if(is.na(f[i])){ #no neutralization
      ...
      pvr[jj, i] <- 1 - exp(-a*(1-q)*(ts[jj]-delay))
