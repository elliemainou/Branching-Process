...
Data1 <- read.csv("TimeToVR/Branching Process/LastUndetFirstDet_C5.csv", header=TRUE)#intermediate
#Data1 <- read.csv("TimeToVR/Branching Process/LastUndetFirstDet_C4.csv", header=TRUE)#early 
...
#Remove althogether individuals who rebound after day 60
# x<-which(FirstDet<60)
# FirstDet<-FirstDet[x]
# LastUndet<-LastUndet[x]

AvgPnorbd <- function(f,a,q0,delay,t){#no rebound function 
  #q=min(q0*f, 1)
  return(exp(-(1-q0*f)*a*(t-delay))*1/f_det)
  #return(exp(-(1-min(q0*f, 1))*a*(t-delay))*1/f_det)
}
...
nloglik <- function(params,FirstDet,LastUndet, Neut_Percent) {
  ...
  if (is.na(t1)) {
    ...
    if(is.na(f)){
      pvr1 <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=params[1],q0=params[2],
                            delay=params[3],t=t1)$value 
      print(pvr1)
    } else {
      q=min(q0*f, 1)
      pvr1 <- 1 - exp(-(1-q)*a*(t1-delay)) 
    }
  }
  if (t2-delay<0) {
    pvr2 = 0
    ...
    if(is.na(f)){
      pvr2 <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=params[1],q0=params[2],
                            delay=params[3],t=t2)$value 
      #print(pvr2)
    } else {
      q=min(q0*f, 1)
      pvr2 <- 1 - exp(-(1-q)*a*(t2-delay))  
    }
  }  
...
### optimization time
x0de <- list(label=c("a", "q0", "delay"),est=c(0.5,0.3, 10),
             low=c(1e-14, 1e-14, 1e-14),upp=c(5, 2, 40))
...
for (jj in c(1:length(ts))) {
  ...
  for(i in 1:animals){
    if(is.na(f[i])){ #no neutralization
      pvr[jj, i] <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=a,q0=q0,
                                  delay=delay,t=ts[jj])$value 
      if (ts[jj]<=delay) {pvr[jj, i]<-0}# if t<delay, pvr=0 for that monkey
    }
    else{ #there is neutralization
      q= min(q0*f[i], 1)
      
      pvr[jj, i] <- 1 - exp(-(1-q)*a*(ts[jj]-delay))
      if (ts[jj]<=delay) {pvr[jj,i]<-0} # if t<delay, pvr=0 for that monkey
    }
  }
}
