...
AvgPnorbd <- function(f,a,q0,delay1,delay2,t){#no rebound function
  q=min(q0*f, 1)
  #delay=delay1+ (delay2-delay1)*f#^2
  delay=delay1+ delay2*f
  #delay=delay1+ (delay1+delay2)*f
  return(exp(-(1-q)*a*(t-delay))*1/f_det)
}
...
nloglik <- function(params,FirstDet,LastUndet, Neut_Percent) {
  ...
  if (is.na(t1)) {
    ...
    if(is.na(f)){
      pvr1 <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=params[1],q0=params[2],
                            delay1=params[3],delay2=params[4],t=t1)$value
    } else {
      q=min(q0*f, 1)
      #delay=delay1+ (delay2-delay1)*f#^2
      delay=delay1+ delay2*f
 ...
 if(is.na(f)){
      pvr2 <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=params[1],q0=params[2],
                            delay1=params[3],delay2=params[4],t=t2)$value
    } else {
      q=min(q0*f, 1)
      #delay=delay1+ (delay2-delay1)*f#^2
      delay=delay1+ delay2*f
...
### optimization time
x0de <- list(label=c("a", "q0", "delay1", "delay2"),est=c(0.01,0.9,5,10),
             low=c(1e-14, 1e-14, 1e-14, 1e-14),upp=c(5, 2, 20, 40))
