...
# AvgPnorbd_CD4_f <- function(x,IPDA,a,delay1,t){# we have IPDA data but no CD4
#   f=x[1]
#   lCD4= x[2]
...
# }
# 
# AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,delay1,t){
#   f=x[1]
#   IPDA=x[2]
#   lCD4=x[3]
...
### optimization time
# x0de <- list(label=c("a","delay1"),est=c(0.01,10),
#              low=c(1e-14,1e-14),upp=c(10,300))
...
for (jj in c(1:length(ts))) {
  ...
  for(i in 1:animals){
    if(is.na(CD4[i]) & !is.na(IPDA[i]) & is.na(f[i])){ #IPDA, no CD4 or f-- double integral 
      ...
      #                                delay1=delay1,t=ts[jj])$integral
      pvr[jj, i] <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                     upperLimit=c(f_det,25),IPDA=IPDA[i],a=a,
      ...
      #                                 a=a,delay1=delay1,t=ts[jj])$integral
      pvr[jj, i] <- 1- adaptIntegrate(AvgPnorbd_CD4_IPDA_f,lowerLimit=c(1e-4,1e-2,15),
                                      upperLimit=c(f_det,Det_IPDA[i],25),Det_IPDA=Det_IPDA[i],
