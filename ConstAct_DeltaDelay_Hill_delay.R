...
AvgPnorbd <- function(f,a,delay1,delay2,k,t){#no rebound function
...
nloglik <- function(params,FirstDet,LastUndet, Neut_Percent) {
  ...
  if (is.na(t1)) {
    ...
    if(is.na(f)){
      pvr1 <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=params[1],
                            delay1=params[2],delay2=params[3],k=params[4],t=t1)$value
    } else {
      delay=delay1+ delay2*f^n/(k^n+f^n)
  ...
  if(is.na(f)){
    pvr2 <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=params[1],
                          delay1=params[2],delay2=params[3],k=params[4],t=t2)$value
  } else {
    delay=delay1+ delay2*f^n/(k^n+f^n)
...
### optimization time
x0de <- list(label=c("a","delay1","delay2","k"),est=c(0.1,3,2, 0.1),
             low=c(1e-14,1e-14,1e-14,1e-14),upp=c(5, 20, 40, 100))
  ...
  for(i in 1:animals){
    if(is.na(f[i])){ #no neutralization
      pvr[jj, i] <- 1 - integrate(AvgPnorbd, lower=0, upper=f_det, a=a,
                                  delay1=delay1,delay2=delay2,k=k,t=ts[jj])$value
