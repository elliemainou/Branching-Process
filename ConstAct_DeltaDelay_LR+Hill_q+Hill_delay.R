...
nq=3
nd=2
AvgPnorbd_CD4_f <- function(x,IPDA,a,kq,kd,delay1,delay2,t){# we have IPDA data but no CD4
  f=x[1]
  lCD4= x[2]
  ...
  q=f^nq/(kq^nq+f^nq)
  delay=delay1+delay2*f^nd/(kd^nd+f^nd)
  return(exp(-a*LR*(1-q)*(t-delay))*pdf_CD4*(1/f_det)) 
}

AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,kq,kd,delay1,delay2,t){
  f=x[1]
  IPDA=x[2]
  lCD4=x[3]
  ...
  if (is.na(t1)) {
    ...
    if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #double integral
      pvr1 <-  1- adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                 ...
                                 kq=params[2],kd=params[3],delay1=params[4],
                                 delay2=params[5],t=t1)$integral
...
### optimization time
x0de <- list(label=c("a","kq","kd","delay1","delay2"),
             est=c(0.1,0.1,0.1,1,1),low=c(1e-14,1e-14,1e-14,1e-14,1e-14),
             upp=c(10,200,200,300,300))
  ...
  for(i in 1:animals){
    if(is.na(CD4[i]) & !is.na(IPDA[i]) & is.na(f[i])){ #IPDA, no CD4 or f-- double integral 
      pvr[jj, i] <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                     ...
                                     kq=kq,kd=kd,delay1=delay1,delay2=delay2,
                                     t=ts[jj])$integral
      ...
      pvr[jj, i] <- 1- adaptIntegrate(AvgPnorbd_CD4_IPDA_f,lowerLimit=c(1e-4,1e-2,15),
                                      ...
                                      a=a,kq=kq,kd=kd,delay1=delay1,delay2=delay2,
                                      t=ts[jj])$integral
      if (pvr[jj,i]<0) {pvr[jj,i]=0}
    }else{ #there is neutralization
      q=f[i]^nq/(kq^nq+f[i]^nq)
      delay=delay1+delay2*f[i]^nd/(kd^nd+f[i]^nd)
