...
AvgPnorbd_CD4_f <- function(x,IPDA,a,delay1,delay2,delay3,delay4,t){# we have IPDA data but no CD4
  f=x[1]
  lCD4= x[2]
...
AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,delay1,delay2,delay3,delay4,t){
  f=x[1]
  IPDA=x[2]
  lCD4=x[3]
...
nloglik <- function(params,FirstDet,LastUndet, IPDA_Data, CD4_Data, 
  ...
  if (is.na(t1)) {
    ...
    if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #double integral
      pvr1 <-  1- adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                 ...
                                 delay3=params[4],delay4=params[5],t=t1)$integral
  ...
  if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #IPDA, no CD4 or f-- double integral 
    pvr2 <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                             upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
                             delay1=params[2],delay2=params[3],
                             delay3=params[4],delay4=params[5],t=t2)$integral
...
### optimization time
# x0de <- list(label=c("a","delay1","delay2","delay3","delay4"),est=c(0.01,5,1,2,0.5),
#              low=c(1e-14,1e-14,1e-14,1e-14,1e-14),upp=c(10,300,300,300,300))
# x0de <- list(label=c("a","delay1","delay2","delay3","delay4"),est=c(0.01,3,1,0.5,-0.5),
#              low=c(1e-14,1e-14,1e-14,1e-14,-300),upp=c(10,300,300,300,300))
x0de <- list(label=c("a","delay1","delay2","delay3","delay4"),est=c(0.01,3,1,0.5,-0.5),
             low=c(1e-14,1e-14,-300,-300,-300),upp=c(10,300,300,300,300))
