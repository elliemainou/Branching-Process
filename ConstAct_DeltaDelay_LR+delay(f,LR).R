...
AvgPnorbd_CD4_f <- function(x,IPDA,a,delay1,delay2,t){# we have IPDA data but no CD4
  f=x[1]
  lCD4= x[2]
  ...
  delay=delay1+delay2*f/LR
  return(exp(-a*LR*(t-delay))*pdf_CD4*(1/f_det))
}

AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,delay1,delay2,t){
  f=x[1]
  IPDA=x[2]
  lCD4=x[3]
  ...
  delay=delay1+delay2*f/LR
  return(exp(-a*LR*(t-delay))*pdf_CD4*(1/Det_IPDA)*(1/f_det))
}

# AvgPnorbd_CD4_f <- function(x,IPDA,a,delay1,delay2,delay3,t){# we have IPDA data but no CD4
#   f=x[1]
#   lCD4= x[2]
#   LR=log10(exp(lCD4)*IPDA/1e6)  #LR size over the entire monkey, take the log10
#   pdf_CD4=(1/(sigma*sqrt(2*pi)))*exp(-(((lCD4)-mu)^2)/(2*sigma^2)) 
#   delay=delay1+delay2*f-delay3*LR
#   return(exp(-a*LR*(t-delay))*pdf_CD4*(1/f_det)) #a=(1-q)*a
# }
# 
# AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,delay1,delay2,delay3,t){ 
#   f=x[1]
#   IPDA=x[2]
#   lCD4=x[3]
#   LR=log10(exp(lCD4)*IPDA/1e6)  
...
nloglik <- function(params,FirstDet,LastUndet, IPDA_Data, CD4_Data, 
  ...
  if (is.na(t1)) {
    ...
    if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #double integral
      pvr1 <-  1- adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                 ...
                                 delay1=params[2],delay2=params[3],t=t1)$integral
      # pvr1 <-  1- adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
      #                            upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
      #                            delay1=params[2],delay2=params[3],
      #                            delay3=params[4],t=t1)$integral
  ...
  if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #IPDA, no CD4 or f-- double integral 
    pvr2 <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                             upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
                             delay1=params[2],delay2=params[3],t=t2)$integral
    # pvr2 <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
    #                          upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
    #                          delay1=params[2],delay2=params[3],
    #                          delay3=params[4],t=t2)$integral
...
### optimization time
x0de <- list(label=c("a","delay1","delay2"),est=c(0.01,5,1),
             low=c(1e-14,1e-14,1e-14),upp=c(10,300,300))
# x0de <- list(label=c("a","delay1","delay2","delay3"),est=c(0.01,5,2,1),
#              low=c(1e-14,1e-14,1e-14,1e-14),upp=c(10,300,300,300))
  ...
  for(i in 1:animals){
    if(is.na(CD4[i]) & !is.na(IPDA[i]) & is.na(f[i])){ #IPDA, no CD4 or f-- double integral 
      pvr[jj, i] <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                     ...
                                     delay1=delay1,delay2=delay2,t=ts[jj])$integral
      # pvr[jj, i] <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
      #                                upperLimit=c(f_det,25),IPDA=IPDA[i],a=a,
      #                                delay1=delay1,delay2=delay2,
      #                                delay3=delay3,t=ts[jj])$integral
      ...
      pvr[jj, i] <- 1- adaptIntegrate(AvgPnorbd_CD4_IPDA_f,lowerLimit=c(1e-4,1e-2,15),
                                      ...
                                      a=a,delay1=delay1,delay2=delay2,t=ts[jj])$integral
      # pvr[jj, i] <- 1- adaptIntegrate(AvgPnorbd_CD4_IPDA_f,lowerLimit=c(1e-4,1e-2,15),
      #                                 upperLimit=c(f_det,Det_IPDA[i],25),Det_IPDA=Det_IPDA[i],
      #                                 a=a,delay1=delay1,delay2=delay2,
      #                                 delay3=delay3,t=ts[jj])$integral
      if (pvr[jj,i]<0) {pvr[jj,i]=0}
    }else{ #there is neutralization
      delay=delay1+delay2*f[i]/LR[i]
      #delay=delay1+delay2*f[i]-delay3*LR[i]
      pvr[jj, i] <- 1 - exp(-LR[i]*a*(ts[jj]-delay))
...
#d=delay1+delay2*f/LR
#d=delay1+delay2*f-delay3*LR
#hist(d)
