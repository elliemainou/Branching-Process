...
AvgPnorbd_CD4_f <- function(x,IPDA,a,q0,delay1,t){# we have IPDA data but no CD4
  f=x[1]
  lCD4= x[2]
  ...
  q=min(q0*f, 1)
  delay=delay1/LR
  return(exp(-a*LR*(1-q)*(t-delay))*pdf_CD4*(1/f_det))
}

AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,q0,delay1,t){
  f=x[1]
  IPDA=x[2]
  lCD4=x[3]
...
nloglik <- function(params,FirstDet,LastUndet, IPDA_Data, CD4_Data, 
  ...
  if (is.na(t1)) {
    ...
    if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #double integral
      pvr1 <-  1- adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                 ...
                                 q0=params[2],delay1=params[3],t=t1)$integral
      # pvr1 <-  1- adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
      #                            upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
  ...
  if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #IPDA, no CD4 or f-- double integral 
    pvr2 <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                             upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
                             q0=params[2],delay1=params[3],t=t2)$integral
    # pvr2 <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
    #                          upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
    #                          q0=params[2],delay1=params[3],delay2=params[4],
    #                          t=t2)$integral
...
### optimization time
x0de <- list(label=c("a","q0","delay1"),est=c(0.01,0.1,10),
             low=c(1e-14,1e-14,1e-14),upp=c(10,2,300))
# x0de <- list(label=c("a","q0","delay1", "delay2"),est=c(0.01,0.1,5,5),
  ...
  for(i in 1:animals){
    if(is.na(CD4[i]) & !is.na(IPDA[i]) & is.na(f[i])){ #IPDA, no CD4 or f-- double integral 
      pvr[jj, i] <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                     ...
                                     q0=q0,delay1=delay1,t=ts[jj])$integral
      # pvr[jj, i] <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
      #                                upperLimit=c(f_det,25),IPDA=IPDA[i],a=a,
      ...
      pvr[jj, i] <- 1- adaptIntegrate(AvgPnorbd_CD4_IPDA_f,lowerLimit=c(1e-4,1e-2,15),
                                      ...
                                      a=a,q0=q0,delay1=delay1,t=ts[jj])$integral
      # pvr[jj, i] <- 1- adaptIntegrate(AvgPnorbd_CD4_IPDA_f,lowerLimit=c(1e-4,1e-2,15),
      #                                 upperLimit=c(f_det,Det_IPDA[i],25),Det_IPDA=Det_IPDA[i],
