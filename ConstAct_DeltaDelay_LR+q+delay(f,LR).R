...
# AvgPnorbd_CD4_f <- function(x,IPDA,a,q0,delay1,delay2,t){# we have IPDA data but no CD4
#   f=x[1]
#   lCD4= x[2]
...
# }
# 
# AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,q0,delay1,delay2,t){ 
#   f=x[1]
#   IPDA=x[2]
#   lCD4=x[3]
...
AvgPnorbd_CD4_f <- function(x,IPDA,a,q0,delay1,delay2,delay3,t){# we have IPDA data but no CD4
  f=x[1]
  lCD4= x[2]
  LR=log10(exp(lCD4)*IPDA/1e6)  #LR size over the entire monkey, take the log10
  pdf_CD4=(1/(sigma*sqrt(2*pi)))*exp(-(((lCD4)-mu)^2)/(2*sigma^2)) 
...
AvgPnorbd_CD4_IPDA_f <- function(x,Det_IPDA,a,q0,delay1,delay2,delay3,t){ 
  f=x[1]
  IPDA=x[2]
  lCD4=x[3]
  LR=log10(exp(lCD4)*IPDA/1e6)  
...
nloglik <- function(params,FirstDet,LastUndet, IPDA_Data, CD4_Data, 
  ...
  if (is.na(t1)) {
    ...
    if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #double integral
      ...
      #                            q0=params[2],delay1=params[3],delay2=params[4],t=t1)$integral
      pvr1 <-  1- adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                 upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
                                 q0=params[2],delay1=params[3],delay2=params[4],
                                 delay3=params[5],t=t1)$integral
  ...
  if(is.na(CD4) & !is.na(IPDA) & is.na(f)){ #IPDA, no CD4 or f-- double integral 
    ...
    #                          upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
    #                          q0=params[2],delay1=params[3],delay2=params[4],t=t2)$integral
    pvr2 <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                             upperLimit=c(f_det,25),IPDA=IPDA,a=params[1],
                             q0=params[2],delay1=params[3],delay2=params[4],
                             delay3=params[5],t=t2)$integral
...
### optimization time
# x0de <- list(label=c("a","q0","delay1", "delay2"),est=c(0.01,0.1,10,5),
#              low=c(1e-14,1e-14,1e-14,1e-14),upp=c(10,2,100,100))
x0de <- list(label=c("a","q0","delay1","delay2","dealy3"),est=c(0.01,0.1,5,2,1),
             low=c(1e-14,1e-14,1e-14,1e-14,1e-14),upp=c(10,2,300,300,300))
...
for (jj in c(1:length(ts))) {
  ...
  for(i in 1:animals){
    if(is.na(CD4[i]) & !is.na(IPDA[i]) & is.na(f[i])){ #IPDA, no CD4 or f-- double integral 
      ...
      #                                q0=q0,delay1=delay1,delay2=delay2,t=ts[jj])$integral
      pvr[jj, i] <- 1-adaptIntegrate(AvgPnorbd_CD4_f, lowerLimit=c(1e-4,15),
                                     upperLimit=c(f_det,25),IPDA=IPDA[i],a=a,
      ...
      #                                 upperLimit=c(f_det,Det_IPDA,25),Det_IPDA=Det_IPDA[i],
      #                                 a=a,q0=q0,delay1=delay1,delay2=delay2,t=ts[jj])$integral
      pvr[jj, i] <- 1- adaptIntegrate(AvgPnorbd_CD4_IPDA_f,lowerLimit=c(1e-4,1e-2,15),
